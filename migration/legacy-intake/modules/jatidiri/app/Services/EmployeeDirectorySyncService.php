...
class EmployeeDirectorySyncService
{
    ...
    public function sync(string $source): array
    {
        return match ($source) {
            ...
            'sikep-portal' => $this->syncFromSikepPortal(),
            default => [
                'ok' => false,
                'inserted' => 0,
                'updated' => 0,
    ...
    private function syncFromSikepPortal(): array
    {
        $loginUrl = trim((string) config('lawangsewu.sikep.portal_login_url', ''));
        $employeeUrl = trim((string) config('lawangsewu.sikep.portal_employee_export_url', ''));
        $username = (string) config('lawangsewu.sikep.portal_username', '');
        $password = (string) config('lawangsewu.sikep.portal_password', '');
        ...
        $verifyTls = (bool) config('lawangsewu.sikep.verify_tls', true);
        $timeout = (int) config('lawangsewu.sikep.timeout_seconds', 15);
        ...
        $csrfField = $this->extractCsrfFieldName($html)
            ?? (string) config('lawangsewu.sikep.portal_csrf_field', '_csrf');
        $csrfValue = $this->extractCsrfValue($html, $csrfField);

        $usernameField = (string) config('lawangsewu.sikep.portal_username_field', 'username');
        $passwordField = (string) config('lawangsewu.sikep.portal_password_field', 'password');
    ...
    {
        $baseUrl = trim((string) config('lawangsewu.sikep.base_url'));
        if ($baseUrl === '') {
            return [
                'ok' => false,
                'inserted' => 0,
        ...
        $endpoint = trim((string) config('lawangsewu.sikep.employee_endpoint', '/pegawai'));
        $token = trim((string) config('lawangsewu.sikep.token', ''));
        $headers = [];
        ...
        try {
            $response = Http::timeout((int) config('lawangsewu.sikep.timeout_seconds', 15))
                ->withOptions([
                    'verify' => (bool) config('lawangsewu.sikep.verify_tls', true),
                ])
                ->acceptJson()
                ->withHeaders($headers)
