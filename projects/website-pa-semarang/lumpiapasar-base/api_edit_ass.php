...
//akta cerai 
  ...
  if(base64_decode($aksi)=='validasi_akta_cerai')
  {
    ...
    $hari_ini=date("Y-m-d");
    $sql="SELECT perkara.para_pihak FROM perkara_akta_cerai LEFT JOIN perkara ON perkara.perkara_id=perkara_akta_cerai.perkara_id WHERE nomor_perkara='$nomor_perkara' AND nomor_akta_cerai='$nomor_akta_cerai'"; //echo $sql;
    $data["req"]=base64_encode($sql);
  ...
  if(base64_decode($aksi)=='cek_penerbitan_akta_cerai')
  {
    ...
    $data["req"]=base64_encode($sql);
    if(strlen(curl($url_api,$data))>=5){
      echo "<h3 class='w3-text-green'>Nomor Perkara ".$nomor_perkara."<br><br>Akta Cerai sudah jadi (dicetak) dan silahkan untuk segera diambil</h3>";
    }else{
...
//antrian sidang  
  ...
  if(base64_decode($aksi)=='antrian_sidang_validasi_nomor_perkara')
  {
    ...
    $hari_ini=date("Y-m-d");
    //$hari_ini="2021-03-22";
    $sql="SELECT nomor_perkara ,para_pihak ,perkara_id ,(SELECT count(perkara_id) FROM perkara_jadwal_sidang WHERE tanggal_sidang='$hari_ini' AND perkara_id=perkara.perkara_id LIMIT 1) AS sidang FROM perkara where nomor_perkara='$nomor_perkara' ";
    //echo $sql;
    $data["req"]=base64_encode($sql);
  ...
  if(base64_decode($aksi)=='proses_antrian_sidang_lama_sipp'){
    $message='';
    $nomor_perkara=trim(base64_decode($_POST["nomor_perkara"]));
    $tekek =date("Y-m-d"); 
    //$tekek="2021-03-22";
    $sql="SELECT 
            ...
            , a.ruangan 
            , b.nomor_perkara 
            , (SELECT nomor_antrian FROM antrian where perkara_id=a.perkara_id AND tanggal_sidang=a.tanggal_sidang LIMIT 1 ) AS nomor_antrian
            , (SELECT nomor_antrian FROM antrian where  tanggal_sidang=a.tanggal_sidang AND ruang_id=a.ruangan_id ORDER BY nomor_antrian ASC LIMIT 1 ) AS nomor_terakhir
            , (SELECT count(perkara_id) FROM perkara_jadwal_sidang WHERE tanggal_sidang='$tekek') AS jumlah_perkara
          FROM perkara_jadwal_sidang AS a
          LEFT JOIN perkara AS b ON b.perkara_id=a.perkara_id
          WHERE b.nomor_perkara='$nomor_perkara' AND a.tanggal_sidang='$tekek' LIMIT 1";
        //echo $sql;exit();
    $data["req"]=base64_encode($sql);
    //cek tidak ada
    ...
    foreach ($hasil as $key => $rentangan){
      ...
      $nomor_antrian=$rentangan->nomor_antrian;
      ...
      {
        $respon = array('status' => 'red','respons'=>'','message'=>'Mohon Perhatian... <br>Ruangan belum ditentukan sehingga belum bisa mengambil antrian','sidang'=>0 );
        echo json_encode($respon);
        exit;
      }else 
      if($nomor_antrian==NULL){
       ...
       // for ($x = 1; $x <= $jumlah_perkara; $x++) {
        ...
        $sql="INSERT INTO antrian(nomor_antrian, tanggal_sidang,  ruang_id,  perkara_id,  status,  urutan) VALUES ('$nomor_antrian', '$tekek',  '$ruangan_id',  '$perkara_id', 0 ,  '$nomor_antrian') "; 
    ...
    if(base64_decode($aksi)=='proses_antrian_sidang'){
        ...
        $sql=
            ...
            , a.ruangan 
            , b.nomor_perkara 
            , (SELECT nomor FROM antriandb.nomorantrian where id_sidang=a.id AND id_sipp=a.perkara_id LIMIT 1 ) AS nomor_antrian
            , (SELECT nomor FROM antriandb.nomorantrian  where  ruang=a.ruangan_id ORDER BY nomor DESC LIMIT 1 ) AS nomor_terakhir
        ...
        foreach ($hasil as $key => $rentangan){
            ...
            if($ruangan_id==NULL OR $ruangan_id==''){
                $respon = array(
                        'status' => 'red',
                        'respons'=>'',
                        'message'=>'Mohon Perhatian... <br>Ruangan belum ditentukan sehingga belum bisa mengambil antrian',
                        'sidang'=>0 
                );
                echo json_encode($respon);
                exit;
                
            }else if($nomor_antrian==NULL){
                $nomor_antrian=(int)$nomor_terakhir+1;
                $sql=
                    "INSERT INTO antriandb.nomorantrian (status, nomor, ruang, nama_ruang, no_perk, id_sipp , id_sidang, penggugat, alamatP, tergugat, alamatT, tgl_sidang) 
                    VALUES (0, $nomor_antrian, $ruangan_id, '$ruangan', '$no_perk', $perkara_id , $id_sidang, '$penggugat', '$alamatP', '$tergugat', '$alamatT', '$tekek') "; 
                //menyimpan ke log antrian sidang SIPP LOKAL PA SMG=================================================================
                $data["req"]=base64_encode($sql); 
                $hasil= json_decode(curl($url_api,$data));
    ...
    if(base64_decode($aksi)=='proses_pilih_nomor_antrian_sidang'){
        ...
        if($hasil=='[]'){
            $sql=
                "INSERT INTO antrian(nomor_antrian, tanggal_sidang,  ruang_id,  perkara_id,  status,  urutan) 
                VALUES ('$nomor_antrian', '$tekek',  '$ruang_id',  '$perkara_id', 0 ,  '$nomor_antrian') "; 
            $data["req"]=base64_encode($sql); 
            $hasil= json_decode(curl($url_api,$data));
...
//kondisi sidang 
  ...
  if(base64_decode($aksi)=='info_persidangan'){
    $tekek=date("Y-m-d");
   $sql = "  SELECT * FROM antriandb.display_antrian_temp WHERE r_sidang<12"; 
