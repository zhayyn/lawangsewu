...
$portalUrl = '/gateway/index';
$landingUrl = lw_public_url('/');
$requestedReturn = gateway_normalize_return_path((string) ($_REQUEST['return'] ?? ''), $portalUrl);
if ($requestedReturn === '/portal' || $requestedReturn === '/lawangsewu/portal') {
    $requestedReturn = $portalUrl;
}

$loginError = gateway_flash_get('auth_error') ?? '';
$submittedUsername = '';
$showLoginPanel = ((string) ($_GET['login'] ?? '') === '1') || $loginError !== '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (string) ($_POST['landing_login'] ?? '') === '1') {
    ...
    $requestedReturn = gateway_normalize_return_path((string) ($_POST['return'] ?? ''), $portalUrl);
    if ($requestedReturn === '/portal' || $requestedReturn === '/lawangsewu/portal') {
        $requestedReturn = $portalUrl;
    }
    $result = gateway_attempt_login($submittedUsername, (string) ($_POST['password'] ?? ''));
    if (!empty($result['ok'])) {
        gateway_flash_set('auth_success', 'Login berhasil. Anda sedang diarahkan ke dashboard portal.');
        header('Location: ' . $requestedReturn);
        exit;
    }

    $baseError = (string) ($result['message'] ?? 'Login gagal.');
    $loginError = $baseError === 'Username atau password salah.'
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .panel-secondary {
            ...
            min-height: 44px;
            border-radius: 14px;
            border: 1px solid rgba(143, 192, 238, 0.16);
            background: rgba(143, 192, 238, 0.06);
            color: rgba(219, 235, 250, 0.92);
            font: inherit;
            font-size: 12px;
            font-weight: 700;
        ...
        .panel-support {
            ...
            padding: 12px 14px;
            border-radius: 14px;
            border: 1px solid rgba(143, 192, 238, 0.14);
            background: rgba(143, 192, 238, 0.05);
            color: rgba(219, 235, 250, 0.84);
            font-size: 12px;
            line-height: 1.65;
        }
        .panel-support.is-visible {
            display: block;
        }
        ...
        .panel-form.is-submitting .panel-submit {
            opacity: 0.84;
            cursor: wait;
        }
        .panel-form.is-submitting input {
            opacity: 0.82;
        }
...
<body>
    ...
    <div class="dock">
        <div class="dock-panel<?php echo $showLoginPanel ? ' is-open' : ''; ?>" id="loginPanel">
            ...
            <?php else : ?>
                ...
                <p class="panel-note">Masuk untuk melanjutkan ke <strong><?php echo htmlspecialchars($requestedReturn, ENT_QUOTES, 'UTF-8'); ?></strong>.</p>
                <?php if ($loginError !== '') : ?>
                    <div class="panel-error"><?php echo htmlspecialchars($loginError, ENT_QUOTES, 'UTF-8'); ?></div>
                <?php endif; ?>
                <form class="panel-form" method="post" action="<?php echo htmlspecialchars($landingUrl, ENT_QUOTES, 'UTF-8'); ?>" id="landingLoginForm">
                    <input type="hidden" name="landing_login" value="1">
                    <input type="hidden" name="return" value="<?php echo htmlspecialchars($requestedReturn, ENT_QUOTES, 'UTF-8'); ?>">
                    ...
                    <button class="panel-submit" type="submit" id="landingLoginButton">Masuk</button>
                    <div class="panel-status" id="landingLoginStatus">Sedang memverifikasi akun dan menyiapkan dashboard...</div>
                </form>
                <button class="panel-secondary" type="button" id="supportAccessButton">Butuh Bantuan Akses?</button>
                <div class="panel-support<?php echo $loginError !== '' ? ' is-visible' : ''; ?>" id="supportAccessPanel">
                    <strong>Bantuan akses portal:</strong><br>
                    Jika akun Anda belum bisa masuk atau password ditolak, hubungi <strong>Tim Dubes Prakom / Administrator Portal</strong> melalui kanal internal yang biasa digunakan di unit kerja Anda.
                </div>
            <?php endif; ?>
        </div>
    ...
    <script>
        (function () {
            ...
            const loginButton = document.getElementById('landingLoginButton');
            const loginStatus = document.getElementById('landingLoginStatus');
            const supportButton = document.getElementById('supportAccessButton');
            const supportPanel = document.getElementById('supportAccessPanel');
            ...
            if (supportButton && supportPanel) {
                supportButton.addEventListener('click', function () {
                    supportPanel.classList.toggle('is-visible');
                });
            }
