...
function buildApiCandidates(limit) {
    return [
        `${window.location.origin}/api/pengumuman-rss?source=all&limit=${limit}`,
        `${window.location.origin}/lawangsewu/api/pengumuman-rss?source=all&limit=${limit}`,
        `${window.location.origin}/widgets/views/php/api/api-pengumuman-rss.php?source=all&limit=${limit}`,
        `${window.location.origin}/lawangsewu/widgets/views/php/api/api-pengumuman-rss.php?source=all&limit=${limit}`,
    ];
}
...
async function loadPengumuman() {
    ...
    try {
        const limit = 5;
        let json = null;
        for (const base of buildApiCandidates(limit)) {
            try {
