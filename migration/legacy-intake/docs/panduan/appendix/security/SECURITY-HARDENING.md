...
Jika tidak ingin membaca seluruh runbook keamanan, mulai dulu dari `SECURITY-GUIDE-LAWANGSEWU.md` atau indeks utama `DOCS-INDEX-LAWANGSEWU.html`.
...
2. Update runtime env di server (jangan commit):
  - `projects/website-pa-semarang/lumpiapasar-s9-active/.env` (folder aktif dengan nama legacy)
   - `gateway/.env`
